...
def test_search_profiles_for_phil_only_fetches_exact_match_details() -> None:
    client = RootMeClient(
        api_key="test",
        base_url="https://api.www.root-me.org",
        request_delay_ms=0,
            ...
            return [
                {
                    ...
                    "1": {"id_auteur": "3788", "nom": "Phil"},
                    "2": {"id_auteur": "4396", "nom": "philippe"},
                    "3": {"id_auteur": "19304", "nom": "PhilShield"},
                },
                {"rel": "next", "href": "https://api.www.root-me.org/auteurs?nom=Phil&debut_auteurs=50"},
            ]
        return [
    ...
    fetched_profile_ids: list[int] = []

    async def fake_get_profiles_by_ids(author_ids: list[int]):
        fetched_profile_ids.extend(author_ids)
        return [
            client._build_profile(
                {
                    "id": author_id,
                    "nom": "Phil",
                    "score": "100",
                    "position": "10",
                    "url": f"https://www.root-me.org/{author_id}",
                },
                validations_payload={"items": [{"id_challenge": 1}]},
                fallback_username="Phil",
            )
            for author_id in author_ids
        ]

    client._request_json = fake_request_json  # type: ignore[method-assign]
    client.get_profiles_by_ids = fake_get_profiles_by_ids  # type: ignore[method-assign]

    import asyncio

    profiles = asyncio.run(client.search_profiles("Phil"))

    assert [profile.id for profile in profiles] == [3788, 24870, 42613]
    assert fetched_profile_ids == [3788, 24870, 42613]
