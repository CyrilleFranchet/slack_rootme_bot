...
If `OLLAMA_MODEL` is also configured, those solve notifications include one extra short joke or encouragement line generated by your local Ollama instance. By default the bot expects Ollama at `http://127.0.0.1:11434`; override that with `OLLAMA_BASE_URL` and adjust the request timeout with `OLLAMA_TIMEOUT_SECONDS`. If Ollama is unavailable, the bot falls back to a built-in message so notifications still go out.
